...
# Look at using factors

describe(train_set)


# Basic representation of data proportions
# Plot showing the first 3 features.
plot_port <- train_set %>% select(school,sex,age) %>% 
  group_by(age,sex,school) %>% summarise(count=n())

ggplot(plot_port) +
  geom_col(aes(x = age, y = ifelse(plot_port$sex== "M",count,-count), fill = sex)) +
  coord_flip() + 
  facet_wrap(~ school) +  
...
G3_min
# Only 1 student scored less the 1 

# Does age have a relationship with G3
train_set %>% ggplot(aes(age,G3))+geom_point() + geom_smooth()
# There isn't enough date in the 20+ region to make and confident conclusions

# Does studytime have a relationship with G3
train_set %>% ggplot(aes(studytime,G3))+geom_point() + geom_smooth()

# Does gender have a relationship with G3
train_set %>% ggplot(aes(sex,G3))+geom_boxplot()
# Slightly lower mean G3 for men
# I have already confirmed the data is reasonably balanced in terms of sex

# Is there a difference in G3 between schools?
train_set %>% ggplot(aes(school,G3))+geom_boxplot()
# MS appears to have lower scores with the mean and quartiles below GP

#Is that a relationship between address and G3?
train_set %>% ggplot(aes(address,G3))+geom_boxplot()
# A slight change however mostly overlapped

#Is that a relationship between famsize and G3?
train_set %>% ggplot(aes(famsize,G3))+geom_boxplot()
# No significant change in G3 between GT3 & LE3
...
# Observation from the residuals vs fit plot
    ...
    # However the data is not equally spread across the score range so ther is some uncertainty.
...
# Observation of Residuals Vs Leverage 
  ...
  mul_reg_pp <- mul_reg_pp[-which(rownames(mul_reg_pp)%in% c("309","373","309")),]
  ...
  skew_adu_results
  #RMSE improved from 2.39 to 2.33
...
# KNN 
  ...
  knn_pred <- predict(knn_fit_tuned, newdata = train_set)
  length(knn_pred,)
  length(train_set$G3)
  RMSE(train_set$G3,knn_pred)
  ...
  plot(svm_fit_tuned)
  #The final value used for the model was C = 0.016 
...
# Ensemble
  ...
  model_list <- caretList(G3 ~ .,
                          ...
                          methodList = c("glmnet", "rf", "knn","svmLinear"),
                          tuneList = NULL,
